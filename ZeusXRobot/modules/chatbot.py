...
from ZeusXRobot import BOT_ID, pgram as pbot, arq
from ZeusXRobot.utils.chatbot import add_chat, get_session, remove_chat
from ZeusXRobot.utils.wtf import admins_only, edit_or_reply
...
async def inuka(client, message):
    ...
    test = test.replace("Zeus", "Aco")
    test = test.replace("My god is @ctzfamily", "I'm a Christian")
    test = test.replace("16", "9")
    test = test.replace("@ctzfamily is my husband.", "I'm single.")
    test = test.replace("@unitedsupport", "Emikobot.ai")
    test = test.replace("I live in @Unitedsupport.", "I live in San Francisco, California.")
    test = test.replace("I was created by @ctzfamily", "I made myself")
    test = test.replace(
        "Hello there I am Zeus...nice to meet u",
        "Hi, my friend! Do you want me to tell you a joke?")
    test = test.replace("@Ctzfamily is my owner", "Have the control right.")
    test = test.replace(
        "Hi, My name is Zeus Nice to meet you.",
        "Hi, my friend, what can I do for you today?")

    response = await lunaQuery(test, message.from_user.id if message.from_user else 0)
    response = response.replace("Aco", "Zeus")
    response = response.replace("aco", "Zeus")
    response = response.replace("Luna", "Zeus")
    response = response.replace("luna", "Zeus")
    response = response.replace("I'm a Christian", "My god is @ctzfamily")
    response = response.replace("9", "16")
    response = response.replace("I'm married to my job.", "I'm working with @ctzfamily")
    response = response.replace("I'm single.", "I'm single god")
    response = response.replace("Emikobot.ai", "@uintedsupport")
    response = response.replace("I live in San Francisco, California.", "I live in @Unitedsupport")
    response = response.replace("I made myself", "I was Created by @ctFamily")
    response = response.replace(
            "Hi, my friend! Do you want me to tell you a joke?",
            "Hello there I am Zeua...nice to meet u")
    response = response.replace("Have the control right.", "@ctzfamily is my owner.")
    response = response.replace(
            "Hi, my friend, what can I do for you today?",
            "Hi, My name is Emiko Nice to meet you")
...
@pbot.on_message(filters.regex("Zeus|zeus|robot|ZEUS|vegeta") & ~filters.bot & ~filters.via_bot  & ~filters.forwarded & ~filters.reply & ~filters.channel & ~filters.edited)
async def inuka(client, message):
    msg = message.text
    if msg.startswith("/") or msg.startswith("@"):
        message.continue_propagation()
    ...
    test = test.replace("Zeus", "Aco")
    test = test.replace("Zeus", "Aco")
    test = test.replace("My god is @ctfamily", "I'm a Christian")
    test = test.replace("16", "9") 
    test = test.replace("I'm single god.", "I'm single.")
    test = test.replace("@Uintedsupport", "Emikobot.ai")
    test = test.replace("I live in @Unitessupport.", "I live in San Francisco, California.")
    test = test.replace("I was created by @ctzfamily", "I made myself")
    test = test.replace(
        "Hello there I am Zeus...nice to meet u",
        "Hi, my friend! Do you want me to tell you a joke?")
    ...
    response = response.replace("luna", "zeus")
    response = response.replace("I'm a Christian", "My god is @ctzfamily")
    response = response.replace("I'm married to my job.", "my friend @Ctzfamily")
    response = response.replace("9", "16") 
    response = response.replace("I'm single.", "I'm single god")
    response = response.replace("Emikobot.ai", "@Unitedsupport")
    response = response.replace("I live in San Francisco, California.", "I live in @emikosupport.")
    response = response.replace("I made myself", "I was Created by @ctzfamily")
    response = response.replace(
            "Hi, my friend! Do you want me to tell you a joke?",
            "Hello there I am Zeus...nice to meet u")
    response = response.replace("Have the control right.", "@ctzfamily is my owner.")
...
__help__ = """
❂ Zeus AI is the only ai system which can detect & reply upto 200 language's
